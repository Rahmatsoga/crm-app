...
export default function Pipeline() {
  ...
  async function load() {
    const [d, c] = await Promise.all([
      supabase.from('deals').select('*, clients(name)').order('created_at', { ascending: false }),
  ...
  async function handleAddDeal(e) {
    ...
    const { error } = await supabase.from('deals').insert([
      { client_id: form.client_id, title: form.title, value: Number(form.value) || 0, stage: 'new' },
    ])
    if (error) {
  ...
  return (
    <div className="p-8">
      ...
      {showForm && (
        <form onSubmit={handleAddDeal} className="bg-white border border-line rounded-xl p-4 mb-6 flex gap-3 flex-wrap">
          <select
      ...
      <div className="grid grid-cols-6 gap-3">
        {STAGES.map((stage) => {
          ...
          return (
            <div key={stage.key} className="bg-white border border-line rounded-xl p-2 min-h-[200px]">
              ...
              <div className="space-y-2">
                {stageDeals.map((d) => (
                  <div key={d.id} className="bg-paper border border-line rounded-lg p-2">
                    <p className="text-xs font-medium mb-0.5">{d.title}</p>
                    <p className="text-[11px] text-ink/50 mb-2">{d.clients?.name}</p>
                    <p className="text-[11px] font-medium mb-2">{d.value ? `$${d.value}` : '—'}</p>
                    <select
                      value={d.stage}
